...
class MessagesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $product1_by_A = Product::where('user_id', $userA->id)->first();
        $product2_by_A = Product::where('user_id', $userA->id)->skip(1)->first();

        // ユーザーBが出品した商品の中からいくつか取得
        $product1_by_B = Product::where('user_id', $userB->id)->first();

        // (出品者) と noProductUser (購入者) の取引メッセージ
