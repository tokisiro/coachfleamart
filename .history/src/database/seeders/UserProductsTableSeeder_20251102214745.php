...
class UserProductsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $sellerBSaleProductIds = Product::where('user_id', $sellerB->id)
            ->where('status', 'sale')
        ...
        // sellerBSaleProductIds からランダムに $targetSoldCount 個の商品IDを選択
        // 選択できる商品数が $targetSoldCount 未満の場合は、選択できるだけ選択
        $productsToSell = collect($sellerBSaleProductIds)->shuffle()->take($targetSoldCount)->all();

        foreach ($productsToSell as $productId) {
            $product = Product::find($productId);
            if (!$product) {
                $this->command->warn("Product ID {$productId} not found during sellerB sold product creation.");
                continue;
            }
