...
class EvaluationsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // user2 が出品し、user1 が購入した取引を強制的に作成し、評価
        $this->createSpecificEvaluation($user2, $user1, 'seller', 'buyer');

        // 必要であれば、さらに別の取引を模擬して評価を増やす
        // user1が再び購入者として評価されるケース
        $this->createSpecificEvaluation($user2, $user1, 'seller', 'buyer', rand(1, 3)); // 敢えて低めの評価も

        // user2が再び購入者として評価されるケース
        $this->createSpecificEvaluation($user1, $user2, 'seller', 'buyer', rand(3, 5)); // 中程度の評価
