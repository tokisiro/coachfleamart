...
class UserProductsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        DB::table('products')->update(['consider_id' => null, 'status' => 'sale']);
    ...
    $sellingProductIds = Product::where('status', 'sale')
        ->whereNull('consider_id')
        ->pluck('id')->toArray();

    // 作成したい「sold」の商品の数を設定
    $numberOfSoldProducts = min(1, count($sellingProductIds));

    // $numberOfSoldProducts の回数だけループして、「sold」の商品と購入履歴を作成
    for ($i = 0; $i < $numberOfSoldProducts; $i++) {
            ...
            // 選ばれた商品の 'consider_id' を購入者IDに設定し、'status' を 'sold' にします。
