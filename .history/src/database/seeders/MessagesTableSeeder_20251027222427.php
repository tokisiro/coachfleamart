...
class MessagesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // '取引中' の商品を取得
        $inTransactionProducts = Product::where('status', '取引中')->get();

        foreach ($inTransactionProducts as $product) {
            $sellerId = $product->user_id; // 出品者
            $buyerId = $product->consider_id; // 購入検討者 (UserProductsTableSeederで設定されたconsider_id)
            ...
            if ($sellerId && $buyerId) {
                ...
                Message::create([
                    ...
                    'read_at' => Carbon::now()->subDays(2)->subHours(2),
                ...
                //     'sender_id' => $sellerId,
                //     'receiver_id' => $buyerId,
                //     'message' => 'ご検討ありがとうございます。ご購入でよろしいでしょうか？',
                //     'read_at' => null,
        ...
        // userAが出品した、まだ取引中でない商品を取得 (例: UserProductsTableSeederで売却済み/取引中にされていないもの)
