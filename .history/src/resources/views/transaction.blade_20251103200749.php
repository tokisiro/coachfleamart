...
<div class="transaction">
    ...
    <div class="transaction-main">
        ...
        {{-- 評価モーダル --}}
        <div id="ratingModal" class="transaction-main__modal" >
            <div class="transaction-main__modal-content">
                <h3 class="transaction-main__modal-content--title">取引が完了しました。</h3>
                <p class="transaction-main__modal-content--messa">今回の取引相手はどうでしたか？</p>
                ...
                {{-- hiddenで評価対象のユーザーIDと役割を送信 --}}
                    ...
                    <input type="hidden" name="reviewed_user_id" id="reviewedUserId" value="">
                    <input type="hidden" name="role_as_reviewed" id="roleAsReviewed" value="">
                    <div class="transaction-main__modal-content--form">
                    ...
                    <button type="submit" class="transaction-main__modal-content--button">送信する</button>
                </form>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const buyerId = {{ $actualBuyer->id ?? 'null' }};
    ...
    // 商品ステータスが 'sold' の場合
    if (productStatus === 'negotiation') {
        // 購入者のみ「取引を完了する」ボタンを表示
        if (currentUserRole === 'buyer') {
            completeTransactionButton.textContent = '取引を完了する';
            completeTransactionButton.style.display = 'block';
        }
        // 出品者は「取引を完了する」ボタンを表示しない
    }
    // 商品ステータスが 'completed' の場合
    else if (productStatus === 'evaluated') {
        // ログインユーザーがまだ評価をしていない場合のみボタンを表示
    ...
    completeTransactionButton.addEventListener('click', function(e) {
        ...
        // case 1: products.status が 'sold' の場合（購入者のみ、APIを叩いてステータスを'completed'にし、評価モーダルを表示）
        if (Status === 'sold') {
            if (currentUserRole === 'buyer') {
                // 取引完了APIを呼び出す
                ...
                .then(data => {
                    if (data.success) {
                        alert(data.message); // 例: 「取引を完了しました。相手を評価してください。」
                ...
                alert('この操作は購入者のみが実行できます。');
            }
        }
        // case 2: products.status が 'completed' の場合（全員が「評価モーダル」を表示）
        else if (productStatus === 'evaluation') {
            if (isCurrentUserRated) {
                alert('あなたはすでに評価済みです。');
                return;
            } else {
    ...
    if (ratingForm) {
        ratingForm.addEventListener('submit', function(e) {
            ...
            .then(result => {
                ...
                if (result.success) {
                    location.reload(); // 評価成功したらページをリロードして最新の状態に更新
                } else {
                    ratingModal.style.display = 'none'; // エラー時もとりあえず閉じる
                }
            })
            .catch(error => {
