...
@section('content')
        <div class="profile-image">
            <div class="profile-image__icon">
                <div class="profile-image__icon-header">
                    <img class="profile-image__icon-header--item" src="{{ $user->icon ? asset('storage/' . $user->icon) : asset('storage/default.png') }}" alt="">
                    <div class="profile-image__icon-header--details">
                        <label class="profile-image__icon-header--details-label">
                            {{ $user->name }}
                        </label>
                        <div class="profile-image__icon-header--details-rating">
                        @php
                        $displayAverageRating = $averageRating ?? 0;
                        $filledStars = floor($displayAverageRating);
                        $emptyStars = 5 - $filledStars;
                        @endphp
                        <div class="profile-image__icon-header--details-rating-item">
                        @php
                        ...
                        @for ($i = 0; $i < $filledStars; $i++)
                            <span class="profile-image__icon-header--details-rating-star filled"></span>
                        @endfor
                        {{-- 空の星の表示 --}}
                        @for ($i = 0; $i < $emptyStars; $i++)
                            <span class="profile-image__icon-header--details-rating-star empty"></span>
                        @endfor
                        </div>
                    </div>
                </div>
                <div class="profile-image__icon-header--link">
                    <a class="profile-image__icon-header--link-item" href="/mypage/profile">
                        プロフィールを編集
                    </a>
                </div>
            </div>
            ...
            @forelse($transactionProducts as $product)
                <a class="profile-list__transaction-card" href="/item/{{ $product->id }}/transaction" >
                    ...
                    <div class="profile-list__transaction-card--content">
                        ...
                        <div class="transaction-status-tag">
                    @if ($product->transaction_status === 'negotiation')
                        <span style="color: #007bff; font-weight: bold;">[交渉中]</span> 
                    @elseif ($product->transaction_status === 'evaluation')
                        <span style="color: #ffc107; font-weight: bold;">[評価待ち]</span> 
                    @elseif ($product->transaction_status === 'evaluated')
                        <span style="color: #fd7e14; font-weight: bold;">[評価確認中]</span> 
                    @endif
                    <span class="product-id-info"> (ID: {{ $product->id }})</span>
                </div>
                    </div>
                </a>
                @empty
