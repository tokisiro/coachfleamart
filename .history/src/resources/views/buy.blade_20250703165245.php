...
@section('content')
    <form class="buy" action="{{ route('product.purchase', ['product' => $detail->id]) }}" method="post">
    @csrf
            ...
            <div class="buy-content__payment">
                ...
                <div class="buy-content__payment-select">
                    <select class="buy-content__payment-select--option" name="payment-method" id="payment-method">
                        <option value="選択してください" class="buy-content__payment-select--option-item" selected hidden>
                            選択してください
                        </option>
    ...
    const paymentSelect = document.getElementById('payment-method');
        ...
        if (paymentSelect && paymentDisplay) {
            paymentSelect.addEventListener('change', () => {
                ...
                paymentDisplay.textContent = selectedMethod || '選択してください';
    });
    // 初期反映：ページ読み込み時にセレクトの値に合わせて表示
    paymentDisplay.textContent = paymentSelect.value || '選択してください';
    }
