...
@section('content')
    <div class="product">
        ...
        <div class="product-tag">
            <a href="/?page=recommendation{{ $searchTerm ? '&query=' . urlencode($searchTerm) : '' }}" class="{{ $mode == 'recommendation' ? 'active' : '' }}"  id="recommendationBtn">
                おすすめ
            </a>
            <a href="/?page=mylist&query={{ $searchTerm ? '&query=' . urlencode($searchTerm) : '' }}" class="{{ $mode == 'liked' ? 'active' : '' }}" id="myListBtn">
                マイリスト
            </a>
        </div>
