...
document.addEventListener('DOMContentLoaded', function() {
    ...
    completeTransactionButton.addEventListener('click', function(e) {
        ...
        if (shouldShowRatingModal) {
            ...
            // 評価対象のユーザーIDと役割を設定（コントローラーで既に設定済みだが念のため再設定）
            let targetReviewedUserId = null;
            let targetRoleAsReviewed = null;

            // ログインユーザーが出品者の場合、相手(購入者)を評価する
            if (currentUserId === sellerId && buyerId !== null) {
                targetReviewedUserId = buyerId;
                targetRoleAsReviewed = 'buyer';
            }
            ...
            // ratingModalはJavaScriptの定数として定義されている前提
            if (ratingModal) {
        ...
        if (Status === 'sold' && productStatus === 'negotiation') {
        ...
        else if (isBuyerRated && currentUserRole === 'seller') {
