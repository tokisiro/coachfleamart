...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addMessageActionListeners(container) {
        // container の中から、全ての「編集」ボタン（クラス名 `btn-edit-message` がついているもの）を探す
        ...
        // container の中から、全ての「削除」ボタン（クラス名 `btn-delete-message` がついているもの）を探す
        const deleteButtons = container.querySelectorAll('.transaction-main__message-chat--item-action-edit');
        deleteButtons.forEach(button => {
            button.removeEventListener('click', handleDeleteClick); // 多重登録防止
            button.addEventListener('click', handleDeleteClick);
        });
    }

    // 既存のすべてのメッセージにイベントリスナーを設定 (初期ロード時)
    ...
    function handleEditClick(event) {
        ...
        // ポップアップやモーダルなどで編集UIを提供しても良いですが、今回はシンプルに prompt を使用
