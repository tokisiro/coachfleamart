...
document.addEventListener('DOMContentLoaded', function() {
    ...
    completeTransactionButton.addEventListener('click', function(e) {
        ...
        // products.status が 'sold' の場合（購入者のみ「取引完了確認モーダル」を表示）
        if (productStatus === 'sold') {
            if (currentUserRole === 'buyer') {
                completeTransactionModal.style.display = 'flex'; // 取引完了確認モーダルを表示
            } else {
                alert('この機能は購入者のみ利用可能です。'); // 出品者がなぜかボタンを押せてしまった場合
            }
        }
