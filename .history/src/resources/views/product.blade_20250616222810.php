...
<main>
    <div class="product">
        ...
        <!--マイリストで表示するリスト-->
        <div class="product-myList" style="display: none;">
            @foreach ($products as $product)
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const likedDiv = document.querySelector('.product-liked'); // 追加
...
// 例：認証ユーザのIDを取得して
$likedProducts = DB::table('nice')
    ->join('products', 'nice.product_id', '=', 'products.id')
    ->where('nice.user_id', '=', Auth::id()) // ログインユーザ
    ->select('products.*')
    ->get();


        // いいねリストだけ表示
        if (likedDiv)likedDiv.style.display = 'block';
        });
    });
</script>
</body>
