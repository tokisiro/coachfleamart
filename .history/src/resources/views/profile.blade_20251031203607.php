        ...
        <div class="profile-rating">
            @if ($averageRating !== null && $evaluationsCount > 0)
                <div class="profile-rating__item">
                    @for ($i = 1; $i <= 5; $i++)
                        <span class="profile-rating__star @if ($i <= $averageRating) filled @else empty @endif"></span> {{-- 塗りつぶされた星 --}}
                    @endfor
                        <span class="profile-rating__item-count">（{{ $evaluationsCount }}件の評価
                        </span>
                </div>
                    <span class="profile-rating__item-count">（{{ $evaluationsCount }}件の評価）</span>
                </div>
