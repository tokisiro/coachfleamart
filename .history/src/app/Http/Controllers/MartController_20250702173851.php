...
class MartController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        // いいねした商品の取得
        $liked_products_query = Product::whereIn('id', function($query) {
            $query->select('product_id')
                ->from('nices')
                ->where('user_id', Auth::id());
            })->where('status', '販売中');
            ...
            if ($searchTerm) {
                $liked_products = $liked_products_query->where('product_name', 'LIKE', "%{$searchTerm}%")->get();
            } else {
                $liked_products = $liked_products_query->get();
            }
