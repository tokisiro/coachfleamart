...
{
    Log::info('post_code: ' . $request->input('post_code'));
Log::info('shipping_address: ' . $request->input('shipping_address'));
Log::info('building: ' . $request->input('building'));
    try {
        ...
        $price = $product->price * 100; // セント単位に変換
        ...
        if ($paymentMethod === 'カード払い') {
            $session = \Stripe\Checkout\Session::create([
                ...
                'shipping_address' => $request->input('shipping_address'), // もしリクエストから
        ...
        // 他の支払い方法に対応する場合はここに追加

        return response()->json(['error' => '不正な支払い方法です'], 400);
    } catch (\Exception $e) {
    ...
    public function complete(Request $request)
{
    ...
    try {
        ...
        // 必要な商品IDなどをStripe Metadataや情報から取得
        $productId = $session->metadata->product_id ?? null;

        // 例：住所や商品名もStripe Metadataに保存している場合
        $shipping_address = $session->metadata->shipping_address ?? '';
        $post_code = $session->metadata->post_code ?? '';
        $building = $session->metadata->building ?? '';

        $product = Product::find($productId);
        // もしsessionに住所情報や商品IDを保存していない場合は、別途
        // 他の方法(セッションやDB)から取得
    ...
    public function completePurchase(Request $request)
{
    ...
    if (!$sessionId) {
        return redirect()->route('top')->with('error', '購入情報が見つかりません。');
    }
    ...
    try {
        ...
        // 顧客情報や商品情報をStripeから取得
        $customer = $session->customer_details;
        $amount = $session->amount_total / 100; // 通貨単位へ変換
        $productName = isset($session->display_items[0]['custom']['name']) ? $session->display_items[0]['custom']['name'] : '商品';

        // 例：購入者IDや商品IDをStripeのMetadataに保存している場合
        // 例：
        // $productId = $session->metadata->product_id;
        // $userId = auth()->id();

        // 実例：そこから取得できる情報をもとに登録
        $productId = 1; // 例：実際はStripe metadataや送信データなどから取得
        $userId = auth()->id();
    ...
    $building = $request->input('building');
        ...
        \Log::error($e->getMessage());
        return redirect()->route('top')->with('error', '購入情報の登録に失敗しました。');
    }
}
    ...
    public function information(AddressRequest $request)
    {
        ...
        if (!$request->session()->has('already_registered')) {
        // セッションにフラグをセット
        $request->session()->put('already_registered', true);
        // 初回だけはログイン画面へ
        
        return redirect('/login');
    } else {
        $request->session()->forget('already_registered');
        // 2回目以降はトップページへ
        return redirect('/');
    }
    }
