    ...
    public function complete(Request $request)
{
    ...
    {
        Log::info('rateProduct called', ['product_object' => $product, 'product_id_from_object' => $product ? $productId->id : 'NULL']);
        // 評価者は現在の認証済みユーザー
        $reviewerId = Auth::id();
        ...
        // 既に評価済みか確認
        $existingEvaluation = Evaluation::where('product_id', $producti->id)
        ...
        try {
            ...
            DB::rollBack();
            // エラーログ
            \Log::error('評価保存エラー: ' . $e->getMessage(), ['exception' => $e, 'reviewer_id' => $reviewerId, 'product_id' => $product->id]);
