    ...
    public function createCheckoutSession(Request $request)
{
    ...
    try {
        ...
        if ($paymentMethod === 'カード払い') {
            ...
            $completeUrl = route('complete') .
    '?session_id=' . urlencode('') .  // session_idなし
    '&address=' . urlencode($request->input('address')) .
    '&post_code=' . urlencode($request->input('post_code')) .
    '&building=' . urlencode($request->input('building')) .
    '&payment_method=' . urlencode($request->input('payment_method'));

// これを返す
return response()->json(['checkoutUrl' => $completeUrl]);
            }
        } catch (\Exception $e) {
