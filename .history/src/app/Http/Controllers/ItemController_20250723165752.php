            ...
            $session = \Stripe\Checkout\Session::create([
                ...
                'line_items' => [[
                    'price_data' => [
                        ...
                        'unit_amount' => min($product->price * 100, 300000 * 100);,
                    ],
                    'quantity' => 1,
                ]],
                'mode' => 'payment',
...
//コンビニ決済完了の通知機能
    ...
    if ($event->type === 'checkout.session.completed') {
        ...
        $this->updatePurchaseHistory($session);
    ...
    private function updatePurchaseHistory($session)
    {
        // 事前にmetadataに入れておいた情報取得
        ...
        // 商品のステータス変更
        \DB::table('products')->where('id', $metadata->product_id)->update([
            'status' => 'sold'
        ]);
    }
