    ...
    public function complete(Request $request)
{
    ...
    public function transaction(Product $product){
        ...
        $otherTransactions = Product::where(function ($query) use ($user, $buyerId) {
                $query->where('user_id', $user->id) // ログインユーザーが出品者
                    ->orWhereHas('buyerUserProduct', function ($q) use ($user) {
    ...
    private function getTransactionPartner(Product $product, $isCurrentUserRated, ?User $buyer)
    {
        if ($product->user_id === $isCurrentUserRated->id) {
            // ログインユーザーが出品者の場合、取引相手は購入検討者
            return $buyer;
        } elseif ($buyer && $buyer->id === $isCurrentUserRated->id) {
