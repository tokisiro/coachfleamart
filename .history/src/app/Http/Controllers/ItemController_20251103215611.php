    ...
    public function complete(Request $request)
{
    ...
    public function completeTransaction(Request $request, $productId)
    {
        ...
        // 商品ステータスが 'sold' でtransaction_statusカラムが'negotiation'の場合 transaction_statusカラムを'evaluated' に変更
    ...
    public function rateProduct(Request $request, Product $product)
    {
        ...
        $validatedData = $request->validate([
            ...
            'rating' => 'required|integer|min:1|max:5', // 1から5の整数
            'role_as_reviewed' => 'required|string|in:seller,buyer', // sellerまたはbuyer
        ]);

        // フォームから送信された product_id と URLの $product->id が一致するか確認
        if ($validatedData['product_id'] != $product->id) {
            return response()->json([
                'success' => false,
                'message' => '無効なリクエストです。'
            ], 400);
        ...
        // 既に評価済みか確認
        $existingEvaluation = Evaluation::where('product_id', $product->id)
        ...
        try {
            ...
            // ここで取引ステータスを更新することも検討 (例: 双方評価完了で'completed'へ)
            // たとえば、評価されたユーザーの役割に基づいて、productのstatusを更新する
            // $product->transaction_status を 'evaluated' から 'completed' に変更するなど
            // 相手側も評価しているか確認する必要がある

            // 例: 買い手が評価したら、売り手が評価済みか確認し、両方評価済みなら取引完了
            // $otherPartyEvaluated = Evaluation::where('product_id', $product->id)
            //                                  ->where('reviewer_id', '!=', Auth::id())
            //                                  ->first();
            // if ($otherPartyEvaluated) {
            //     $product->update(['transaction_status' => 'completed']);
            // } else {
            //     // まだ片方しか評価していない場合
            //     // 必要に応じて何か処理
            // }
