    ...
    public function completePurchase(Request $request)
{
    ...
    try {
        ...
        $amount = $session->amount_total / 100; // 通貨単位に戻す
        $productName = $session->display_items[0]['custom']['name'] ?? '商品';
        ...
        return view('purchase.complete'); // 購入完了ページを表示
    } catch (\Exception $e) {
        \Log::error($e->getMessage());
        ...
        if ($event->type === 'checkout.session.completed') {
            ...
            // sessionのmetadataから商品IDやユーザーIDを取得
            $productId = $session->metadata->product_id;
            // もしユーザー情報もmetadataに入れてたら取得
            // $userId = $session->metadata->user_id; など
            ...
            DB::transaction(function() use ($productId) {
                ...
                DB::table('user_products')->insert([
                    ...
                    'user_id' => auth()->id(), // 必要に応じて
                    'post_code' => '', // 必要なら追加
                    'shipping_address' => '', // 必要なら追加
                    'building' => '', // 必要なら追加
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
