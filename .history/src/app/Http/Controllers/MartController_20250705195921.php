...
class MartController extends Controller
{
    ...
    public function profile(Request $request){
        ...
        $tab = $request->query('tab', 'purchase');
        $user = auth()->user();
        ...
        $purchasedProducts = [];
         ...
         view('profile', compact('user', 'purchasedProducts', 'listedProducts'));
    }
}
    ...
    \DB::transaction(function () use ($form) {
        ...
        if (isset($form['category_ids']) && is_array($form['category_ids'])) {
            foreach ($form['category_ids'] as $categoryName) {
                $category = \DB::table('categories')->where('categreturnory', $categoryName)->first();
                if (!$category) {
