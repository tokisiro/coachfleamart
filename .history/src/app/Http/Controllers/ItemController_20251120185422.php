...
use App\Mail\SellerTransactionCompletedNotification;
    ...
    public function complete(Request $request)
{
    ...
    public function profile(Request $request)
    {
        ...
        $query->where('transaction_status', 'negotiation')
        ->orWhere('transaction_status', 'evaluation')
        ->orWhere('transaction_status', 'evaluated');
        })
        //  評価が未完了の商品のみを取得するロジック ★★★
        // 評価テーブルに、ログインユーザーが評価者(reviewer_id)としてレコードが存在しない商品を抽出
    ...
    public function transaction(Product $product){
        ...
        $product->load('buyerUserProduct.user','considerUser');

        // 正式な購入者を取得
        $actualBuyer = $product->buyer;

        // 購入予定者を取得
        $considerUser = $product->considerUser;
        ...
        // 正式な購入者がおらず、購入予定者がいる場合、そのユーザーは当事者
        else if ($considerUser) {
        $isRelatedToProduct = $isRelatedToProduct || ($considerUser->id === $user->id);
        }
        ...
        // その他の取引中の商品を取得
        $otherTransactions = Product::whereIn('transaction_status', ['negotiation', 'evaluation'])
            ->where('id', '!=', $product->id) // 現在表示中の商品を除く
            ->where(function ($query) use ($user) {
                // ログインユーザーが出品者である商品
                $query->where('user_id', $user->id)
                    ->orWhere('consider_id', $user->id);
        ...
        if ($product->user_id === $user->id) { // ログインユーザーが出品者
            $transactionPartner = $actualBuyer ?? $considerUser?? $chatPartner; // 購入者がいれば購入者、いなければチャット相手
        } elseif ($actualBuyer && $actualBuyer->id === $user->id) { // ログインユーザーが正式な購入者
            $transactionPartner = $product->user; // 取引相手は出品者
        } elseif ($considerUser && $considerUser->id === $user->id) { // ログインユーザーが購入予定者
            $transactionPartner = $product->user;
        ...
        if ($user->id === $product->user_id) {
            // ログインユーザーが出品者 (reviewer) の場合、評価対象は購入者 (reviewed_user)
            if ($actualBuyer) {
                $reviewedUserId = $actualBuyer->id;
                $roleAsReviewed = 'buyer'; // 購入者として評価される
            }
        } elseif ($actualBuyer && $user->id === $actualBuyer->id) {
            // ログインユーザーが購入者 (reviewer) の場合、評価対象は出品者 (reviewed_user)
            $reviewedUserId = $product->user_id; // Productモデルに出品者のIDがある前提
            $roleAsReviewed = 'seller'; // 出品者として評価される
        }
    }
        ...
        return view('transaction', [
            ...
            'reviewedUserIdForModal' => $reviewedUserId,
            'roleAsReviewedForModal' => $roleAsReviewed,
    ...
    //JS（フロントエンド）に"取引完了ボタン"押下の結果レスポンスを送る
    public function completeTransaction(Request $request, $productId)
    {
        ...
        if (!$userProduct || $userProduct->user_id !== $user->id) {
            return response()->json(['success' => false, 'message' => 'この商品は未購入のため、評価は行えません。'], 403);
        }
        ...
        if ($product->status === 'sold' && $product->transaction_status === 'negotiation') {
            ...
            // 出品者を取得
            $seller = $product->seller; // Productモデルにsellerリレーションがあることを前提

            // もし出品者が存在し、メールアドレスを持っている場合のみ送信
            if ($seller && $seller->email) {
                // メールを送信
                Mail::to($seller->email)->send(new SellerTransactionCompletedNotification($product, $user));
            }
    ...
    {
        Log::info('rateProduct called', ['product_object' => $productId, 'product_id_from_object' => $productId->id ?? 'NULL']);
        // 評価者は現在の認証済みユーザー
        $reviewerId = Auth::id();
        ...
        try {
            ...
            $evaluationData = [
                ...
                'role_as_reviewed' => $request->role_as_reviewed,
            ];

            Log::info('Attempting to create Evaluation with data', $evaluationData);
            ...
            DB::rollBack();
            // エラーログ
            \Log::error('評価保存エラー: ' . $e->getMessage(), ['exception' => $e, 'reviewer_id' => $reviewerId, 'product_id' => $productId->id]);

            return response()->json([
                'success' => false,
                'message' => '評価の送信中にエラーが発生しました。'
            ], 500);
        }
    }
}
