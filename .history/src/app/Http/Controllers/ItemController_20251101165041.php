    ...
    public function complete(Request $request)
{
    ...
    public function transaction(Product $product){
        ...
        // 評価済みステータスの判定
        // 出品者が購入者を評価済みか
        $isSellerRatedBuyer = Evaluation::where('product_id', $product->id)
            // 出品者が評価者
            ...
            ->exists();
        // 購入者が出品者を評価済みか
        $isBuyerRatedSeller = Evaluation::where('product_id', $product->id)
             // 購入者が評価者
        ...
        // ログインユーザーが「出品者」の場合の評価済みステータス
        $isCurrentUserRated = false;
        if ($user->id === $product->user_id) {
            $isCurrentUserRated = $isSellerRatedBuyer;
        }
        // ログインユーザーが「購入者」の場合の評価済みステータス
        else if ($user->id === $buyerId) {
            $isCurrentUserRated = $isBuyerRatedSeller;
        }
        ...
        return view('transaction', [
            ...
            'buyer' => $buyer, // 購入者UserモデルをBladeに渡す
