...
public function handleWebhook(Request $request)
    {
        $payload = $request->getContent(); // ペイロード（Stripeから送信された情報）
        $sigHeader = $request->header('Stripe-Signature'); // 署名

        // 以下にStripeのWebhook用の秘密鍵を設定（.envに書いておくのが安全）
        $endpointSecret = env('STRIPE_WEBHOOK_SECRET');

        // Stripe SDKを使って署名確認とイベント取得
        try {
