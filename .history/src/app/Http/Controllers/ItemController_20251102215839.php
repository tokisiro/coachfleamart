...
dd($averageRating);
    ...
    public function transaction(Product $product){
        ...
        $chatPartnerId = Message::where('product_id', $product->id)
            ...
            ->flatten() // 配列を平坦化 (例: [1, 2, 3])
            ->unique() // 重複を削除
            ->except($user->id) // ログインユーザー自身のIDを除外
            ->first(); // 複数あっても最初の1つ（この商品に対する取引相手は一人と想定）

            $buyer = null; // 購入者（検討者）を指す変数として利用
        if ($chatPartnerId) {
            $buyer = User::find($chatPartnerId);
        }

        // 現在の商品の出品者または購入検討者（取引相手）がログインユーザーであることを確認
        if ($product->user_id !== $user->id && (!$buyer || $buyer->id !== $user->id)) {
        ...
        if ($isTransactionCompleted && $transactionPartner) {
            ...
            if ($user->id === $product->user_id && $buyer) { // ログインユーザーが出品者の場合
                $isCurrentUserRated = Evaluation::where('product_id', $product->id)
                    ->where('reviewer_id', $user->id) // 出品者が評価者
                    ->where('reviewed_user_id', $buyer->id) // 購入者が被評価者
                    ->exists();
            }
            // ログインユーザーが購入者であれば、評価対象は出品者
            elseif ($buyer && $user->id === $buyer->id) { // ログインユーザーが購入者の場合
                $isCurrentUserRated = Evaluation::where('product_id', $product->id)
        ...
        if ($buyer) { // $buyerが特定できた場合のみ
            $isBuyerRated = Evaluation::where('product_id', $product->id)
                ->where('reviewer_id', $buyer->id) // 購入者が評価者
                ->where('reviewed_user_id', $product->user_id) // 出品者が被評価者
                ->exists();
        }
