    ...
    public function complete(Request $request)
{
    ...
    public function profile(Request $request)
    {
        ...
        if ($mode === 'buy') {
            ...
            // 取引中の商品
            // 以下の2つのパターンを考慮
            // 1. 自分が「出品者」として、他のユーザーと取引中の商品 (products.user_id = 自分 && products.status = '取引中')
            // 2. 自分が「購入者」として、他のユーザーから取引中の商品 (products.buyer_id = 自分 && products.status = '取引中')
            $transactionProducts = Product::where(function ($query) use ($user) {
                $query->where('user_id', $user->id) // 自分が「出品者」の場合
                      ->orWhere('buyer_id', $user->id); // 自分が「購入者」の場合
            })
            ->where('status', '取引中') // productsテーブルのstatusが「取引中」であるもの
            ->get();
