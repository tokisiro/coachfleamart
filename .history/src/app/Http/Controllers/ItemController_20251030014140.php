    ...
    public function complete(Request $request)
{
    ...
    public function transaction(Product $product){
        ...
        $otherTransactions = Product::where(function ($query) use ($user) {
                $query->where('user_id', $user->id) // ログインユーザーが出品者
                      ->orWhere('consider_id', $user->id); // ログインユーザーが購入検討者
            })
    ...
    public function messages(Request $request, Product $product)
    {
        ...
        $messageData = [
            ...
            'message' => $validatedData['message'] ?? null, // バリデーション済みのデータを使用
        ];

        if ($request->hasFile('image')) {
            $image = $request->file('image');
            $filename = uniqid() . '.' . $image->getClientOriginalExtension();
            // storage/app/public/chat_images に保存
            $path = $image->storeAs('public/chat_images', $filename);
            // データベースには公開URLを保存
        ...
        $senderIcon = $message->sender->icon ? asset('storage/' . $message->sender->icon) : asset('images/default_profile.png');

        return response()->json([
            'message' => $message,
            'sender_name' => $message->sender->name,
            'sender_icon' => $message->sender->icon ? asset('storage/' . $message->sender->icon) : asset('storage/default.png'),
