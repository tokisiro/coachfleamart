    ...
    public function complete(Request $request)
{
    ...
    public function transaction(Product $product){
        ...
        $chatParticipants = Message::where('product_id', $product->id)
            ...
            // chatParticipantsにIDが存在すれば、それが取引相手のID
        if ($chatParticipants->isNotEmpty()) {
            // 複数の相手がいる可能性もあるが、ここでは最初のIDを取引相手とする
            // または、特定の商品に対するチャットは一対一を想定
            $buyerId = $chatParticipants->first();
