...
use Laravel\Fortify\Http\Controllers\Controller
...
class MartController extends Controller
{
    ...
    \DB::transaction(function () use ($form) {
        ...
        // カテゴリー登録（複数選択の対応）
        if (isset($form['category_ids[]']) && is_array($form['category[]'])) {
            foreach ($form['category_ids[]'] as $cat) {
                // カテゴリ名からカテゴリーIDを取得（存在しなければ新規作成）
...
class AuthenticatedSessionController extends Controller{
    ...
    public function store(LoginLoginRequest $request)
    {
