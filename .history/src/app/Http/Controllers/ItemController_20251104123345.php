    ...
    public function complete(Request $request)
{
    ...
    public function rateProduct(Request $request, Product $productId)
    {
        $validatedData = $request->validate([
            // 'product_id' はURLから取得するので、ここではバリデーションしないか、
            // もしリクエストボディにも含まれるなら、exists と同じ値であることを確認する
            // 'product_id' => 'required|integer|exists:products,id', // オプショナル: クライアント側からも受け取る場合
            // 'reviewer_id' はサーバーサイドで Auth::id() を使うので、リクエストからは受け取らない
            'reviewed_user_id' => 'required|exists:users,id',
            'rating' => 'required|integer|min:1|max:5',
            'role_as_reviewed' => 'required|string|in:seller,buyer',
            'comment' => 'nullable|string|max:500', // コメントを追加
        ]);
