    ...
    public function complete(Request $request)
{
    ...
    public function transaction(Product $product){
        ...
        $otherTransactions = Product::where(function ($query) use ($user, $buyerId) {
            ...
            ->where('status', ['sold', 'completed']) // ステータスが「取引中」
        ...
        // 取引完了ステータスの判定 (products.status === 'completed' かどうか)
        $isTransactionCompleted = ($product->status === 'completed');
    ...
    public function completeTransaction(Request $request, Product $product)
{
    $currentUserId = Auth::id();
    $buyerUserProduct = $product->buyerUserProduct()->first();
    $buyerId = $buyerUserProduct ? $buyerUserProduct->user_id : null;

    if ($product->user_id !== $currentUserId && $buyerId !== $currentUserId) {
        return response()->json(['message' => 'あなたはこの取引の当事者ではありません。'], 403);
    }

    if ($product->status === 'sold') {
        $product->status = 'completed';
        $product->save();
        return response()->json(['message' => '取引を完了しました。', 'success' => true]);
    } elseif ($product->status === 'completed') {
        return response()->json(['message' => 'この取引はすでに完了しています。', 'success' => false]);
    } else {
        return response()->json(['message' => 'この商品はまだ取引中ではありません。', 'success' => false]);
    }
}

    // 評価送信API
    // evaluations テーブルにレコードを挿入する
    public function submitRating(Request $request, Product $product)
{
    $currentUserId = Auth::id();
    $reviewedUserId = $request->input('reviewed_user_id'); // 評価されたユーザーID
    $rating = $request->input('rating');
    $comment = $request->input('comment');
    $roleAsReviewed = $request->input('role_as_reviewed'); // 評価された側の役割 (buyer or seller)
    ...
    $request->validate([
        ...
        'role_as_reviewed' => 'required|in:buyer,seller', // 'buyer' または 'seller' のいずれか
    ]);

    // ログインユーザーが商品の出品者か購入者か確認
    $buyerUserProduct = $product->buyerUserProduct()->first();
    $buyerId = $buyerUserProduct ? $buyerUserProduct->user_id : null;
    ...
    if ($product->status !== 'completed') {
        return response()->json(['message' => '取引が完了していないため評価できません。', 'success' => false], 400);
    }

    // 評価者が評価対象者を評価する正しい関係かを確認
    $reviewerId = $currentUserId; // 評価者はログインユーザー

    // 評価された側のユーザーIDと役割が正しいか
    if ($roleAsReviewed === 'buyer' && $reviewedUserId !== $buyerId) {
        return response()->json(['message' => '評価対象の購入者IDが不正です。', 'success' => false], 400);
    }
    if ($roleAsReviewed === 'seller' && $reviewedUserId !== $product->user_id) {
        return response()->json(['message' => '評価対象の出品者IDが不正です。', 'success' => false], 400);
    }

    // 評価者が評価された側と逆の役割か確認
    // 出品者が評価者なら、評価された側は購入者
    if ($reviewerId === $product->user_id && $roleAsReviewed !== 'buyer') {
        return response()->json(['message' => '出品者は購入者のみ評価できます。', 'success' => false], 400);
    }
    // 購入者が評価者なら、評価された側は出品者
    if ($reviewerId === $buyerId && $roleAsReviewed !== 'seller') {
        return response()->json(['message' => '購入者が出品者のみ評価できます。', 'success' => false], 400);
    }

    // すでに評価済みか確認 (ユニーク制約と一致させる)
    if (Evaluation::where('product_id', $product->id)
                    ->where('reviewer_id', $reviewerId)
                    ->where('reviewed_user_id', $reviewedUserId)
                    ->exists()) {
        return response()->json(['message' => 'あなたはすでにこの相手を評価済みです。', 'success' => false], 400);
    }
    ...
    Evaluation::create([
        ...
        'rating' => $rating,
        'comment' => $comment,
        'role_as_reviewed' => $roleAsReviewed,
    ]);

    return response()->json(['message' => '評価を送信しました。', 'success' => true]);
    }
    ...
    {
        $product = Product::with('seller', 'buyer')->findOrFail($productId);
        $user = Auth::user();
        $messages = Message::where('product_id', $productId)->with('sender')->get();

        // 取引相手の特定
        $transactionPartner = null;
        if ($product->seller_id === $user->id) {
            // ログインユーザーが販売者であれば、購入者が取引相手
            $transactionPartner = $product->buyer;
        } elseif ($product->buyer_id === $user->id) {
            // ログインユーザーが購入者であれば、販売者（出品者）が取引相手
            $transactionPartner = $product->seller;
        }

        // isTransactionCompleted は、statusが'completed'かどうか
        $isTransactionCompleted = ($product->status === '評価済み');

        // ログインユーザーが「取引相手」を既に評価済みかどうかを判定
        $isCurrentUserRated = false;
        // 取引が完了している場合 かつ 取引相手が特定できている場合のみ評価済みの可能性をチェック
        if ($isTransactionCompleted && $transactionPartner) {
            $existingEvaluation = Evaluation::where('product_id', $productId)
                ->where('reviewer_id', $user->id) // ログインユーザーが評価者
                ->where('reviewed_user_id', $transactionPartner->id) // 取引相手が被評価者
                ->exists();
            if ($existingEvaluation) {
                $isCurrentUserRated = true;
            }
        }
        $buyer = User::find($product->buyer_id);

        $otherTransactions = Product::where(function($query) use ($user) {
